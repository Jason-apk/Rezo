...
import { runFullImportPipeline } from "../importPipeline";
import { clearAllTables, createSchema, isImported, markImported, openDb } from "../schema";
import { DijkstraResult, Graph, GtfsRoute, GtfsStop } from "../types";
...
export const useGtfsStore = create<GtfsState>((set, get) => ({
    ...
    initialize: async () => {
        ...
        try {
            ...
            const stops = await db.getAllAsync<GtfsStop>("SELECT * FROM stops ORDER BY stop_name ASC");
            const routes = await db.getAllAsync<GtfsRoute>("SELECT * FROM routes ORDER BY route_short_name ASC");
